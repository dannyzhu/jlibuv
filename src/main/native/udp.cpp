...
static void _recv_cb(uv_udp_t* udp, ssize_t nread, const uv_buf_t *buf, const struct sockaddr* addr, unsigned flags) {
  ...
  assert(handle->data);
  UDPCallbacks* cb = reinterpret_cast<UDPCallbacks*>(handle->data);
  cb->on_recv(nread, (uv_buf_t *)buf, (struct sockaddr*)addr, flags);
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_UDPHandle__1bind
  ...
  if (r){
    ...
    unsigned flags = 0;
    r = uv_udp_bind(handle, (struct sockaddr*)&addr, flags);
    if (r) {
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_UDPHandle__1bind6
  ...
  if (r){
    ...
    unsigned flags = 0;
    r = 0;//uv_udp_bind6(handle, addr, flags); //TODO
    if (r) {
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_UDPHandle__1send
  ...
  if (r){
    ...
    if (data) {
      ...
      req->data = req_data;
      r = uv_udp_send(req, handle, &buf, 1, (struct sockaddr*)&addr, _send_cb);
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_UDPHandle__1send6
  ...
  if (r){
    ...
    req->data = req_data;
    r = 0;//uv_udp_send6(req, handle, &buf, 1, addr, _send_cb);
    env->ReleasePrimitiveArrayCritical(data, base, 0);
    if (r) {
      delete req_data;
      delete req;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_UDPHandle__1recv_1start
  ...
  if (r /*&& uv_last_error(handle->loop).code != UV_EALREADY*/) { //TODO
