...
static void _fs_cb(uv_fs_t* req) {
  ...
  if (req->result == -1) {
    cb->fs_cb(request, req->fs_type, req->path, 0);//req->errorno); //TODO
  } else {
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1close__JILjava_lang_Object_2Ljava_lang_Object_2
  ...
  if (callback) {
    ...
    req->data = new FileRequest(cb, callback, fd, NULL, context);
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_close"); //TODO
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1open
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (fd == -1) {
      ThrowException(env, -1/*uv_last_error(cb->loop()).code*/, "uv_fs_open", NULL, cpath);
    }
  }
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1read
  ...
  if (callback) {
    uv_fs_t* req = new uv_fs_t();
    FileRequest* request = new FileRequest(cb, callback, fd, NULL, context);
    request->get_bytes(buffer, data, static_cast<jsize>(offset), static_cast<jsize>(length));
    req->data = request;
    jbyte* base = request->bytes();
    uv_buf_t buf = uv_buf_init((char*)(base + offset), (unsigned int)length);
    r = uv_fs_read(cb->loop(), req, fd, &buf, 1, position, _fs_cb);
  } else {
    uv_fs_t req;
    if (data) {
      jbyte* base = new jbyte[length];
    uv_buf_t buf = uv_buf_init((char*)(base), (unsigned int)length);
      r = uv_fs_read(cb->loop(), &req, fd, &buf, 1, position, NULL);
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_read");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1unlink
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_unlink", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return r;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1write
  ...
  if (callback) {
    ...
    if (data) {
      ...
      OOME(env, base);
      uv_buf_t buf = uv_buf_init((char*)(base + offset), (unsigned int)length);
      r = uv_fs_write(cb->loop(), req, fd, &buf, 1, position, _fs_cb);
    ...
    if (data) {
      ...
      OOME(env, base);
      uv_buf_t buf = uv_buf_init((char*)(base + offset), (unsigned int)length);
      r = uv_fs_write(cb->loop(), &req, fd, &buf, 1, position, NULL);
      env->ReleasePrimitiveArrayCritical(data, base, 0);
    } else {
      jbyte* base = (jbyte*) env->GetDirectBufferAddress(buffer);
      uv_buf_t buf = uv_buf_init((char*)(base + offset), (unsigned int)length);
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_write");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1mkdir
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_mkdir", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return r;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1rmdir
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_rmdir", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return r;
...
JNIEXPORT jobjectArray JNICALL Java_com_oracle_libuv_Files__1readdir
  ...
  if (callback) {
    ...
    if (r >= 0) {
        ...
        ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_readdir", NULL, cpath);
    }
    uv_fs_req_cleanup(&req);
  }
  env->ReleaseStringUTFChars(path, cpath);
...
JNIEXPORT jobject JNICALL Java_com_oracle_libuv_Files__1stat
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_stat", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return stats;
...
JNIEXPORT jobject JNICALL Java_com_oracle_libuv_Files__1fstat
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_fstat");
    }
  }
  return stats;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1rename
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      int code = r;//uv_last_error(cb->loop()).code;
      if (dst_path != NULL &&
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1fsync
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_fsync");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1fdatasync
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_fdatasync");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1ftruncate
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_ftruncate");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1sendfile
  ...
  if (callback) {
    uv_fs_t* req = new uv_fs_t();
    req->data = new FileRequest(cb, callback, in_fd, NULL, context);
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_sendfile");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1chmod
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_chmod", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return r;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1utime
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_utime", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return r;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1futime
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_futime");
    }
  }
  return r;
}
...
JNIEXPORT jobject JNICALL Java_com_oracle_libuv_Files__1lstat
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_lstat", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return stats;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1symlink
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_symlink", NULL, src_path);
    }
  }
  env->ReleaseStringUTFChars(path, src_path);
  env->ReleaseStringUTFChars(new_path, dst_path);
...
JNIEXPORT jstring JNICALL Java_com_oracle_libuv_Files__1readlink
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_readlink", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return link;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1fchmod
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_fchmod");
    }
  }
  return r;
}
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1chown
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_chown", NULL, cpath);
    }
  }
  env->ReleaseStringUTFChars(path, cpath);
  return r;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_Files__1fchown
  ...
  if (callback) {
    ...
    uv_fs_req_cleanup(&req);
    if (r < 0) {
      ThrowException(env, r/*uv_last_error(cb->loop()).code*/, "uv_fs_fchown");
    }
  }
  return r;
}
