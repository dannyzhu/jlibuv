...
static void _read2_cb(uv_pipe_t* pipe, ssize_t nread, const uv_buf_t* buf, uv_handle_type pending) {
  ...
  if (pending == UV_TCP) {
    ...
    cb->on_read2((uv_buf_t*)buf, size, reinterpret_cast<jlong>(tcp), pending);
    ...
    cb->on_read2((uv_buf_t*)buf, size, reinterpret_cast<jlong>(p), pending);
    ...
    cb->on_read2((uv_buf_t*)buf, size, reinterpret_cast<jlong>(udp), pending);
  } else {
...
JNIEXPORT void JNICALL Java_com_oracle_libuv_handles_StreamHandle__1read2_1start
  ...
  assert(stream);
  uv_stream_t* handle = reinterpret_cast<uv_stream_t*>(stream);
  int r = 0;//uv_read2_start(handle, _alloc_cb, _read2_cb); //TODO
  if (r) {
