...
#include "stream.h"
#include "context.h"
#include "com_oracle_libuv_handles_TCPHandle.h"
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_TCPHandle__1bind
  ...
  sockaddr_in addr = {0};
  int r = uv_ip4_addr(h, port, &addr);
  if (r) {
    ThrowException(env, handle->loop, "uv_ip4_addr", h);
  }else{
    r = uv_tcp_bind(handle, (struct sockaddr*)&addr, 0);
    if (r) {
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_TCPHandle__1bind6
  ...
  const char* h = env->GetStringUTFChars(host, 0);
  sockaddr_in6 addr = {0};
  int r= uv_ip6_addr(h, port, &addr);
  if (r) {
    ThrowException(env, handle->loop, "uv_ip6_addr", h);
  }else{
    int r = 0;//uv_tcp_bind(handle, &addr); //TODO
    if (r) {
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_TCPHandle__1connect
  ...
  if (r){
    ...
    req->data = req_data;
    int r = uv_tcp_connect(req, handle, (struct sockaddr*)&addr, _tcp_connect_cb);
    if (r) {
      delete req_data;
      delete req;
...
JNIEXPORT jint JNICALL Java_com_oracle_libuv_handles_TCPHandle__1connect6
  ...
  if (r){
    ...
    ContextHolder* req_data = new ContextHolder(env, context);
    req->data = req_data;
    int r = 0;//uv_tcp_connect6(req, handle, addr, _tcp_connect_cb); //TODO
    if (r) {
      delete req_data;
      delete req;
